...
def c_route(
    ...
    ****NOTE: viaoffset pushes both vias towards each other slightly
    extra_vias = adds extra vias at intermediate nodes if metal width permits
    """
    if debug:
        pass
    ...
    #adds via array if any of the two dimensions of the metal route is at least twice of the respective dimension of a single via. 
    if extra_vias:  
        ...
        if use_arr2:
            ...
            if round(edge1.orientation) == 90 or round(edge1.orientation) == 270:
                viastack2 = via_array(pdk, e2glayer, cglayer, size=(width2,cwidth), fullbottom=fullbottom, no_exception=True)
    ...
    e2_extension_comp.add(me2)
    route_ports = [None,None]
    if round(edge2.orientation) == 0 or round(edge2.orientation) == 180:
        via_flush1 = snap_to_grid(abs((width1 - evaluate_bbox(viastack1)[1])/2) if viaoffset else 0)
    if round(edge2.orientation) == 90 or round(edge2.orientation) == 270:
        via_flush1 = snap_to_grid(abs((width1 - evaluate_bbox(viastack1)[0])/2) if viaoffset else 0)

    if round(edge2.orientation) == 0 or round(edge2.orientation) == 180:
        via_flush2 = snap_to_grid(abs((width2 - evaluate_bbox(viastack2)[1])/2) if viaoffset else 0)
    if round(edge2.orientation) == 90 or round(edge2.orientation) == 270:
        via_flush2 = snap_to_grid(abs((width2 - evaluate_bbox(viastack2)[0])/2) if viaoffset else 0)

    via_flush1 = via_flush1 if viaoffset[0] else 0-via_flush1
